...
scripts/shadow.py -- each one is a different set of arguments, never a second code path
...
until that is resolved. The baseline is reproducible rather than stored.
    ...
    python scripts/bench_shade_ladder.py                 # 14:00, the quoted hour
    python scripts/bench_shade_ladder.py --hour 9
    python scripts/bench_shade_ladder.py --day           # every hour 06..20, day mean
"""
...
from shadow import sun_position, shade_factor, RAY_STEP, BEAM
import provenance
...
def anchor_paths(hour):
    ...
    try:
        ...
        from server.engine import _shade_path
