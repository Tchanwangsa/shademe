"""Validation for scripts/svf.py: analytic cases, convergence, radius, real-DSM sanity.

Run: python scripts/test_svf.py [a|b|c|d|e|f]   (default: all)
...
Analytic reference (ENGINE_CONTRACT.md): infinite symmetric canyon, wall height H,
...
from svf import svf_raster, _rays, N_AZ, MAX_R_M, RAY_STEP, Z_PED
...
def part_d(path=None):
    ...
    # scripts/canopy_svf.py), falling back to svf_all. Pinning svf_all here would audit
    ...
    if not os.path.exists(path):
        print(f"  ! {path} missing -- run `python scripts/svf.py` first"); return
...
def _oracle():
    if not ORACLE or not os.path.exists(ORACLE):
        print(f"  ! oracle not found (set SVF_ORACLE=/path/to/svf_oracle.py); skipping")
        return None
