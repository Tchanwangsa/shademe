"""Checks for the Open-Meteo diurnal bias correction in server/weather.py.

Run: python scripts/test_weather_bias.py
...
`python scripts/validate_sensors.py --fit-bias`, which writes its own skill table into
...
def load(**env):
    """Re-import server.weather under a given env, so module-level flags are re-read."""
    keep = {k: os.environ.get(k) for k in ("SHADEME_BIAS", "SHADEME_BIAS_LEVEL")}
    for k in keep:
        os.environ.pop(k, None)
    os.environ.update({k: v for k, v in env.items()})
    import server.weather as w
    importlib.reload(w)
