...
Writes out/material_id.npy (uint8, 0 = default) and out/material_props.json.
  ...
  python scripts/materials.py          # build + validate
"""
...
# Additive hooks read by scripts/surface_temp.py. Both default to 0, which
# leaves the march exactly the ENGINE_CONTRACT equation.
#
...
# ------------------------------------------------------------------- main
def run_check(P):
    """python scripts/materials.py --check -- full-sun diurnal march per class."""
    wx = demo_weather()
    H = wx["hourly"]
