...
from config import BBOX, WGS84, MGA55
...
BIAS_JSON = os.path.join(DATA, "openmeteo_bias.json")  # fitted diurnal correction, read by server/weather.py
...
def fit_diurnal_bias(j, var="airtemperature"):
    ...
    all -- and this app ships a winter mode. Per-season is never harmful in any season.
...
def evaluate_bias_correction(j, fit, var="airtemperature", parkish=("ICTMicroclimate-01", "aws5-0999")):
    ...
    server/weather.py and this table is the argument for revisiting that, not a footnote.
...
def fit_bias_main():
    """`python scripts/validate_sensors.py --fit-bias` -- fit, evaluate, write."""
    fetch_all()
    print("\nloading sensors ...")
    d = load_sensors()
    s = sites(d)
...
# The two models scored against each other. Same shipped code path in scripts/shadow.py,
# different arguments -- exactly the rungs of scripts/bench_shade_ladder.py, so a number
...
def _lux_shade(l, boards, models=("legacy", "slab")):
    ...
    """
    from shadow import point_shade, RAY_STEP, BEAM
...
def lux_verdict(out, beam, models):
    ...
    print("\n=== VERDICT")
    print(f"  The instrument has no power here, and that is the result.")
...
def main():
    ...
    print("  above are the RAW feed; server/weather.py ships the corrected one.")
