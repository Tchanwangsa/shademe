...
"""
import os, sys, json, pickle, hashlib, numpy as np
...
def _shade_path(hour, mode="summer"):
    for d in SHADE_DIRS.get(mode, SHADE_DIRS["summer"]):
...
def solve(E, wx_block, wx_full, hour, K=0.06, g=None):
    ...
    ts_edge = E.get("tsurf", {}).get(int(hour))
    tsurf_c = (ts_edge - 273.15) if ts_edge is not None else np.full(len(E["length"]), ta + 8.0)
...
def attach_tsurf(E, G, wx, g=None, mode="summer", cache=True):
    """Sample the hourly Ts rasters onto edges. Sets E['tsurf'][hour] -> (n,) float in K.
