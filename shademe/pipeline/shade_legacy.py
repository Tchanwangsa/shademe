"""The LEGACY hourly shade set: flat 8 m canopy, no crown base, written into out/.

Superseded by shade.py for everything the engine reads. It is kept because it is the
bottom rung of tools/bench_shade_ladder.py, and because to_png() lives here and both
generators share it.

    python -m shademe.pipeline.shade_legacy [YYYY-MM-DD]
"""
...
def wgs84_bounds(bounds):
    """MGA55 bbox -> WGS84 corners. The grid is ~1.2 deg off true north (grid
    convergence), so hand the client 4 real corners plus an enveloping bbox."""
...
if __name__ == "__main__":
    ...
    for hh in HOURS:
        ...
        # Zeros == the legacy crown-to-pavement model. shade.py passes the real base.
